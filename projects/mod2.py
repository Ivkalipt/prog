...
def reopen(imgX, code):
    ...
    imgX.save(f"./trash/{code}")
    imgX1 = Image.open(f"./trash/{code}.png")
    return imgX1
...
class MainWindow(QMainWindow):
    ...
    def OnGenerate(self):
        ...
        print(os.path.exists(pathToSave))
        if not (os.path.exists(nameOfTitle) and os.path.exists(nameOfAns) and os.path.exists(pathToSave)):
            errw = ErrorWindow(self)
            errw.show()
            return
        else:
