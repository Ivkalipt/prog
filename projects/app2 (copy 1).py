        ...
        try:
            ...
            with open(full_path, mode="r", encoding=Encoding) as file:
                ...
                for row in reader:
                    ...
                    student = Student(
                        code=str(class_num[:-1]).zfill(4) + str(class_counts[class_num]).zfill(3),
                    ...
                    # print(student.code, student.last_name) 
    ...
    def generate_names_table(self):
        ...
        try:
            with open("codes.csv", "r", encoding=Encoding) as infile, \
                ...
                headers = next(reader)
                headers[1] = "ФИО"  # Заменяем "Код" на "ФИО"
                writer.writerow(headers)
                
                # Сопоставляем коды с именами
                code_to_name = {s.code: f"{s.last_name} {s.first_name} {s.sur_name}" 
